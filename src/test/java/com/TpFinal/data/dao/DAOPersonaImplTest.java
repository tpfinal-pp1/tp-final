...
public class DAOPersonaImplTest {
    ...
    private DAO<Propietario> daoprop;
	...
	boolean b = false;
	for (Persona p : dao.readAll()) {
	    b = b || p.getEstadoRegistro().equals(EstadoRegistro.BORRADO);
	}
	assertTrue(b);
	...
	for (RolPersona rp : dao.readAllActives().get(0).getRoles()) {
	    r = rp.giveMeYourRole();
	}
