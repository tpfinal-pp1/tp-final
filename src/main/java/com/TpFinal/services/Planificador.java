...
public class Planificador {
	...
	public void encender() {
		try {
			if (sc != null && !sc.isShutdown()) {
				sc.shutdown(); // bajamos en planificador viejo, si existe.
			}
	...
	private void agregarJobNotificacionCita(Cita c, Integer horas, Integer nro) {
		...
		agregarJobNotificacionSistema(c.getTitulo(), c.getMessage(), username, fechaInicio, fechaFin, perioricidad,
				triggerKey);
	}
	...
	private void agregarJobMailCobroPorVencido(Cobro c, Integer nro) {
		LocalDateTime fechaInicio = LocalDateTime.of(c.getFechaDeVencimiento().plusDays(1), LocalTime.now().plusMinutes(
				1));
		...
		String texto = "Señor/a inquilino/a: " + ca.getInquilinoContrato().getPersona().toString()
	...
	private void agregarJobMailCobroPorVencer(Cobro c, Integer dias, Integer nro) {
		...
		String texto="Señor/ra inquilino/a: "+ca.getInquilinoContrato().getPersona().toString()+" recuerde que el pago del alquiler del inmueble: \n"
	...
	private void agregarJobMailAlquilerPorVencer(ContratoAlquiler c, Integer meses, Integer key) {
		LocalDateTime fechaInicio = LocalDateTime.of(c.getFechaIngreso().plusMonths(c.getDuracionContrato()
				.getDuracion()),
		...
		String mensaje = "Señor/ra inquilino/a "+c.getInquilinoContrato().getPersona().toString()
				+".\nEl contrato de alquiler del inmueble:\n"
				+c.getInmueble().toString()+"\n"
				+"se vence el dia: " + c.getFechaIngreso().plusMonths(c.getDuracionContrato().getDuracion())
				.format(new DateTimeFormatterBuilder().appendPattern("dd/MM/YYYY").toFormatter()).toString() + "."
				+ "\nRecuerde que el inmueble debe ser "
				+ "entregado en condiciones";
		agregarJobMail("Vencimiento de contrato", mensaje, c.getInquilinoContrato().getPersona().getMail(), fechaInicio,
				fechaFin, perioricidad, triggerKey);
	}
