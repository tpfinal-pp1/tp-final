...
public class CobroService {
	...
	calcularDatosFaltantes(ret.stream().filter(c -> c.getContrato() instanceof ContratoAlquiler).collect(Collectors
		.toList()));
	return ret;
    }
	...
	if (propietario.getMail() == null || propietario.getMail().length() == 0)
	    ...
	    new MailSender().enviarMail(propietario.getMail(), "Pago atrasado", "El inquilino " + ca
		    .getInquilinoContrato().getPersona().toString() + "\n "
