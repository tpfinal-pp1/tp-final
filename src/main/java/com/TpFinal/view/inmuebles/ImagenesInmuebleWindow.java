...
public abstract class ImagenesInmuebleWindow extends Window {
   ...
   private VerticalLayout root;
    //private CheckBox isPortada=new CheckBox(null);
   UploadButton upload = new UploadButton(new UploadReceiver() {
        @Override
        public void onSuccessfullUpload(String filename) {
            if (filename != null && filename != "") {
                System.out.println("fILENAME:" + filename);
                inmueble.addPathImagen(filename);
                refreshListSelect();

                //preview.setSource(InmuebleService.GenerarStreamResource(filename));
   ...
   private void resize(){
    ...
    private void refreshListSelect(){
       ...
       if(inmueble.getPathImagenes().iterator().hasNext()) {
           String primera=inmueble.getPathImagenes().
                   iterator().next();
           preview.setSource(InmuebleService.
                   GenerarStreamResource(
                           primera
                   ));
           if(onlyOne){
