...
public class ContratoAlquilerForm extends FormLayout {
	...
	cbInmuebles.addValueChangeListener(new HasValue.ValueChangeListener<Inmueble>() {
		...
		if (valueChangeEvent != null) {
			...
			if (contratoAlquiler.getEstadoContrato() == EstadoContrato.EnProcesoDeCarga) {
				...
				Integer cantidad = asociado.getCantCertificadosGarantes();
				if (cantidad.compareTo(ParametrosSistemaService.getParametros()
					.getCantMinimaCertificados()) < 0)
				    cantidad = ParametrosSistemaService.getParametros().getCantMinimaCertificados();
				contratoAlquiler.setCantCertificadosGarantes(cantidad);
	...
	save.addClickListener(e -> {
		...
		checkFieldsPerTab(e1.getFieldValidationErrors());

	    } catch (ContratoServiceException e2) {
	...
	if (estadoCargaDocumento.equals(EstadoCargaDocumento.Cargando)) {
	    Notification.show("Aguarde a que el documento finalice de cargar e intente nuevamente.",
		    Notification.Type.WARNING_MESSAGE);
	    throw new IllegalStateException("El documento esta cargandose");
	} else {
	    boolean success = false;
