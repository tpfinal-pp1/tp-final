...
public class CobrosABMView extends DefaultLayout implements View {
	...
	private void configureGrid() {
	    ...
	    grid.addColumn(cobro -> {
		String ret = "";
		ret = cobro.getTipoCobro() != null ? cobro.getTipoCobro().toString() : "";
		return ret;
	...
	private Component filtroTipos() {
	    ComboBox<TipoCobro> fitroTipos = new ComboBox<>();
	    fitroTipos.setStyleName(ValoTheme.COMBOBOX_BORDERLESS);
	    fitroTipos.setPlaceholder("Sin Filtro");
	    fitroTipos.setItems(TipoCobro.values());
	    fitroTipos.addValueChangeListener(e -> {
		if (e.getValue() != null) {
		    if (!fitroTipos.isEmpty())
			filtro.setfiltroTipo(cobro -> cobro.getTipoCobro().equals(e.getValue()));
		    else
			filtro.setfiltroTipo(cobro -> true);
		} else {
		    filtro.setfiltroTipo(cobro -> true);
		}
		updateList();
	...
	private Component filtroInquilinos() {
		...
		if (e.getValue() != null) {
		    filtro.setFiltroInquilino(cobro -> {
			ContratoAlquiler c1 = (ContratoAlquiler) cobro.getContrato();
			return c1.getInquilinoContrato()
				.toString().toLowerCase()
				.contains(e.getValue().toLowerCase());
		    });
		} else {
