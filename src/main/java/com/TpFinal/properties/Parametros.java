...
public class Parametros {
	...
	private static final String PARAM_FILE = "Files"+File.separator+"inmobi.properties";
	...
	public static String getProperty(String key)throws IllegalArgumentException, FileExistsException{
		...
		try {
			...
			if (!f.exists()) {
				...
				checkAndCreateDefaultsIfMissing();
				if(!f.exists())
					throw new IllegalArgumentException("Error Fatal al crear las Properties por defecto, problema de privilegios de escritura?" );
				else{
					getProperty(key);
				}
		...
		if(key.equals("emailKey")||key.equals("mapsKey"))
